...
class NonConvexContactHandler(ContactHandler):
    ...
    def __call__(self, args, geom1, geom2, gamma = 1):
        ...
        if is_circle_g1 and is_circle_g2:
            ...
            if is_ncvx_g1 or is_ncvx_g2:
                ...
                best_dist, best_pt1, best_pt2, best_normal = self.resolve_ncvx(world.facets, b1, b2)
                if is_circle_g2:
    ...
    def resolve_ncvx(facets, b1, b2):
        ...
        pos_f0 = torch.matmul(irot.transpose(0,1),torch.tensor(facets[0][0].copy()).float().view(2,1)).view(2,1) + b2.pos.float().view(2,1)
        best_pt1 = 0*b1_wrt_b2
